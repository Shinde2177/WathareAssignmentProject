...
const GetData = () => {
  ...
  const [timeSlotSize, setTimeSlotSize] = useState(1);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://localhost:3001/data?startDate=${startDate}&startHour=${startHour}&timeSlotSize=${timeSlotSize}`);
      if (!response.ok) {
  ...
  return (
    <Container fluid>
      ...
      <Link to="/UploadData" className="btn btn-primary mr-10">Upload Data</Link>
      <Link to="/GetAllData" className="btn btn-primary mr-10">Get All Data</Link>
      ...
      <label>
        Time Slot Size:
        <select value={timeSlotSize} onChange={(e) => setTimeSlotSize(e.target.value)}>
          {Array.from({ length: 24 }, (_, i) => i).map(hour => (
